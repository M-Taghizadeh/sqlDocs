...
/* 9 */
select Student.Student_ID, Student.Student_Name, Student.Student_Family, sum(Lesson_Unit)as'تعداد واحد انتخاب شده'
,sum(case when Score >= 10 then Lesson_Unit else 0 end)as'تعداد واحد پاس شده'
,sum(case when Score < 10 and Score > 0.25 then Lesson_Unit else 0 end)as'تعداد واحد افتاده'
from Student, SelectionUnit, LessonGroup, Lesson
where Student.Student_ID = SelectionUnit.Student_ID and SelectionUnit.Identity_ID = LessonGroup.Identity_ID and LessonGroup.Lesson_ID = Lesson.Lesson_ID
and Student.Student_ID = '96100100'
group by Student.Student_ID, Student_Name, Student_Family
